...
# --- 2. GHL LIVE FETCH (STABLE METHOD) ---
def get_live_ghl_count():
    ...
    # Using 'dateUpdated' with 'gte' as a fallback
    payload = {
        "locationId": LOCATION_ID,
        "filters": [{"field": "dateUpdated", "operator": "gte", "value": start_time.strftime("%Y-%m-%dT%H:%M:%S.000Z")}]
    }
    
    try:
        r = requests.post(url, headers=headers, json=payload, timeout=10)
        contacts = r.json().get('contacts', [])
        # Strict Python filter for the 'client' tag
...
# --- 4. DATA FETCHING (NO CACHE) ---
def fetch_sales_data(sheet, start_time, end_time=None):
    try:
        # Force a fresh pull from Google
...
if client:
    ...
    col2.metric("Yesterday (Sheet)", len(previous_sales))

    st.divider()
    
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("New Sales:")
        for s in current_sales: st.write(f"✔ {s['name']}")
    with c2:
        st.subheader("Yesterday's Sales:")
        for s in previous_sales: st.write(f"• {s['name']}")
else:
    st.error("Google Auth Failed. Please check Render variables.")
